...
public class POFilterQueryBuilder implements FilterQueryBuilder{
	...
	public String getQuery(Iterator conditions) {
            ...
            return select.substring(0,from.length()-1) + " " +from.substring(0,from.length()-1) + " " + where.substring(0,where.length()-2)+" "+groupBy.substring(0,groupBy.length()-1)+" "+having.substring(0,having.length()-1);
	}
}
